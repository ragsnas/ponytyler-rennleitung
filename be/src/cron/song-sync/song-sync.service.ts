...
import {Race, Song} from "@prisma/client";
import {from, combineLatest, firstValueFrom, map, Observable, of} from "rxjs";
    ...
    async handleCron() {
        ...
        if (songsFromCloud && localSongs) {
            songsFromCloud.data.forEach(song => {
                ...
                if(!localSongs.some((localSong: Song) =>
                    this.cleanSongname(this.songToString(localSong)) === this.cleanSongname(fullCloudSongName))) {
