...
export class DbBackupService {
  ...
  private createBackup() {
    ...
    // make actual backup
    this.logger.log(
      `Copying prisma/rl.db to ${destinationPath}/${backupFileName}`,
    );
    copyFileSync(
      `${this.prismaFolder}/rl.db`,
      `${destinationPath}/${backupFileName}`,
    );
    copyFileSync(
      `${this.prismaFolder}/rl.db`,
      `${this.backupFolder}/most_recent_backup.db`,
    );
  }
  ...
  private getLastBackupFileFilename() {
    ...
    if (backupDirectoryContent && backupDirectoryContent.length > 0) {
      ...
      const backupDirectoryYearContent = readdirSync(
        `${directory}/${yearFolder}`,
      )
        .sort()
        .reverse()
        .filter((name) => isNumber(Number(name)))
      ...
      if (backupDirectoryYearContent && backupDirectoryYearContent.length > 0) {
        ...
        const backupDirectoryMonthContent = readdirSync(
          `${directory}/${yearFolder}/${monthFolder}`,
        )
          .sort()
          .reverse()
          .filter((name) => isNumber(Number(name)))
          .filter((name) =>
            isDir(`${directory}/${yearFolder}//${monthFolder}/` + name),
          );

        if (
          backupDirectoryMonthContent &&
          backupDirectoryMonthContent.length > 0
        ) {
          const filePatternRegex = /^(\d){2}-(\d){2}\.db$/;
          const dayFolder = backupDirectoryMonthContent[0];
          const backupDirectoryDayContent = readdirSync(
            `${directory}/${yearFolder}/${monthFolder}/${dayFolder}`,
          )
